...
def convert_gms_to_ounce(weight_in_gms):
    return float(float(weight_in_gms) * 0.035274)


def convert_ounce_to_gms(weight_in_ounce):
    return int(float(float(weight_in_ounce) * 28.3495))


def get_cache_key(request, key):
    url = request.get_full_path() + '_%s' % (key)
    u_hash = hashlib.md5()
    u_hash.update(url)
    return u_hash.hexdigest()


def get_cache_var(request, c_key):
    key = get_cache_key(request, c_key)
    return cache.get(key)


def set_cache_var(request, c_key, obj):
    key = get_cache_key(request, c_key)
    cache.set(key, obj, settings.CACHE_TIMEOUT)
...
def expire_view_cache(path, key_prefix):
    request = HttpRequest()
    request.path = path
...
def compare_dates_between(start_date, end_date):
    ...
    start_date = datetime.datetime.strptime(
        str(start_date), "%Y-%m-%d %H:%M:%S+00:00")
    end_date = datetime.datetime.strptime(
        str(end_date), "%Y-%m-%d %H:%M:%S+00:00")
